...
public class SurvivorResourceIntTest {
	...
	public static Survivor createEntity(EntityManager em) {
		Survivor survivor = new Survivor().name(DEFAULT_NAME).age(DEFAULT_AGE);
		// Add required entity
	...
	public void createSurvivor() throws Exception {
		...
		restSurvivorMockMvc.perform(post("/api/survivors").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(survivorDTO))).andExpect(status().isCreated());
	...
	public void createSurvivorWithExistingId() throws Exception {
		...
		restSurvivorMockMvc.perform(post("/api/survivors").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(survivorDTO))).andExpect(status().isBadRequest());
	...
	public void getAllSurvivors() throws Exception {
		...
		restSurvivorMockMvc.perform(get("/api/survivors?sort=id,desc")).andExpect(status().isOk())
	...
	public void getSurvivor() throws Exception {
		...
		// Get the survivor
		restSurvivorMockMvc.perform(get("/api/survivors/{id}", survivor.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	private void defaultSurvivorShouldBeFound(String filter) throws Exception {
		restSurvivorMockMvc.perform(get("/api/survivors?sort=id,desc&" + filter)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(survivor.getId().intValue())))
				.andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME.toString())))
				.andExpect(jsonPath("$.[*].age").value(hasItem(DEFAULT_AGE)));
	...
	public void updateSurvivor() throws Exception {
		...
		restSurvivorMockMvc.perform(put("/api/survivors").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(survivorDTO))).andExpect(status().isOk());
	...
	public void deleteSurvivor() throws Exception {
		...
		// Get the survivor
		restSurvivorMockMvc
				.perform(delete("/api/survivors/{id}", survivor.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
